...
describe('admin runtime config', () => {
  ...
  it('never reads the consumer app\'s __DRINKSAVER_CONFIG__ global', async () => {
    // Set the consumer app's config but not the admin app's config
    ...
    // Admin should use its own default, not the consumer app's value
    expect(config.keycloakClientId).toBe('drinksaver-admin');
  });
});
