...
class AlcoholNameCollectorTest {
    ...
    /**
     * The volume is rendered with %.2f, which follows the default locale unless
     * told otherwise. Under a comma-decimal locale that silently turns "0.50l"
     * into "0,50l", so the format is pinned to Locale.ROOT instead.
     */
    @Test
    void rendersTheVolumeWithADotRegardlessOfTheDefaultLocale() {
        ...
        try {
            ...
            assertThat(result.name()).isPresent();
            assertThat(result.name().get()).isEqualTo("Vodka (Half litre - 0.50l)");
        } finally {
