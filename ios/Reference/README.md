...
| Browser | Playwright Chromium (version pinned by `web/package.json` at the frozen SHA) |
...
# 5. Install Playwright in the worktree's web directory
cd web && npm install && npx playwright install chromium

# 6. Run the capture (writes into the worktree's ios/Reference/web/)
cd web && npx playwright test --config=e2e/playwright.config.ts ios-reference.spec.ts
...
```bash
docker-compose -p drinksaver-ios-ref down -v
git worktree remove /Users/alexmolnar/personal/sandbox/compare/kimi-ios-ref
```
